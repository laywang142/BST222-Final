...
print(assocstats(contingency_table)$cramer)       


contingency_table2 <- table(df$race, df$smoke)
print(chisq.test(contingency_table2))
print(assocstats(contingency_table2)$cramer)         ## highest association!!!

contingency_table3 <- table(df$poverty, df$smoke)
print(chisq.test(contingency_table3))
print(assocstats(contingency_table3)$cramer)
```
...
# stratified by race
none_mod <- coxph(surv~1, data=df) ##model with only intercept
full_mod <- coxph(surv~poverty*smoke*alcohol*agemth*yschool*pc3mth+strata(race), data=df) ##first order model with all predictors 
...
```{r}
# stratified by race
cox9 <- coxph(surv ~ smoke+poverty+strata(race), data=df)
summary(cox9)
AIC(cox9)
```
...
```{r}
# stratified by smoke
cox10 <- coxph(surv ~ smoke+strata(race), data=df)
summary(cox10)
AIC(cox10)
```

```{r}
cox_r <- coxph(surv ~ race, data=df)
summary(cox_r)
AIC(cox_r)

cox_s <- coxph(surv ~ smoke, data=df)
summary(cox_s)
AIC(cox_s)

cox_p <- coxph(surv ~ poverty, data=df)
summary(cox_p)
AIC(cox_p)

cox_y <- coxph(surv ~ yschool, data=df)
summary(cox_y)
AIC(cox_y)
```
